...
class LibreLinkUp: Logging {
    ...
    var regionalSiteURL: String {
        "https://" + (
            settings.libreLinkUpRegion == "cn" ? "api-cn.myfreestyle.cn" :
                settings.libreLinkUpRegion == "ru" ? "api.libreview.ru" :
                "api-\(settings.libreLinkUpRegion).libreview.io"
        )
    }
    ...
    func login() async throws -> (Any, URLResponse) {
        ...
        if let storefront = await Storefront.current {
            ...
            if let country = countryISOCodes[countryCode] {
                ...
                // TODO:
                settings.libreLinkUpInstallationId = "0057f7f5-41ee-4416-b4bc-18a841dfba0c" // TEST
                let headers = [
                    "X-Installation-ID": settings.libreLinkUpInstallationId,
                                   ...
                                   let token = ticketDict["token"] as? String {
                                    ...
                                    if let json = try JSONSerialization.jsonObject(with: data) as? [String: Any] {
                                        ...
                                        if let status = json["status"] as? Int,
                                           ...
                                           let message = error["message"] as? String {
                                            log("LibreView: error: \(message), status: \(status)")
                                            // TODO
                                        }
